...
from route_optimization_research.multi_poi.algorithms.simulated_annealing import (
    simulated_annealing
)
...
def run_benchmark():
    ...
    for _, case in cases.iterrows():
        ...
        poi_ids = parse_list(case["poi_ids"])

        start_poi = str(case["start_poi_id"])
        end_poi = str(case["end_poi_id"])

        matrix = load_matrix(case_id)

        # -----------------------------------------------------
        # Normalize matrix IDs to strings
        # -----------------------------------------------------
    ...
    df.to_csv(
        os.path.join(
            OUTPUT_DIR,
            "simulated_annealing_benchmark.csv"
        ),
        index=False
    )
    ...
    by_size.to_csv(
        os.path.join(
            OUTPUT_DIR,
            "simulated_annealing_by_size.csv"
        ),
        index=False
    )
    ...
    by_profile.to_csv(
        os.path.join(
            OUTPUT_DIR,
            "simulated_annealing_by_profile.csv"
        ),
        index=False
    )
    ...
    print("\n" + "=" * 70)
    print("SIMULATED ANNEALING BENCHMARK SUMMARY")
    print("=" * 70)

    print(f"Total cases       : {summary['total_cases']}")
    print(f"Feasible cases    : {summary['feasible_cases']}")
    print(f"Infeasible cases  : {summary['infeasible_cases']}")
